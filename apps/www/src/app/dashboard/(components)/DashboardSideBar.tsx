  ...
  return (
    <div className="mb-1">
      ...
      {section.items.map((item) => {
        const href = buildHref(workspaceSlug, item.segment);
        return (
          <SidebarNavItem
...
export default function DashboardSideBar() {
  ...
  const config: NavConfig = workspaceNav;
  ...
  useEffect(() => {
    ...
    for (const section of config.main) {
      ...
      for (const child of section.children) {
        const href = buildHref(workspaceSlug, child.segment);
  ...
  }, [pathname, config.main, workspaceSlug, activeMenuId, setActiveMenuId]);
