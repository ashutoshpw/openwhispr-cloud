 ...
 *
 * DATAFORSEO_AUTH holds the pre-built `Basic <base64>` Authorization header
 * value (generate locally with `echo -n 'login:password' | base64`).
 */
...
export interface SerpRankInput {
  keyword: string;
  /** Domain to look for in results, e.g. "example.com" */
  targetDomain: string;
  /** ISO country code, default "US" */
  country?: string;
  /** Language code, default "en" */
  languageCode?: string;
  /** Max depth to scan (DataForSEO billing scales with depth) */
  depth?: number;
}
...
/**
 * Single live SERP rank check via DataForSEO's `live/regular` endpoint.
 * ~$0.003 per request. Returns null position when domain is not in results.
 */
