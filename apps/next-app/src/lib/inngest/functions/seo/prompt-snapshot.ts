/**
 * Weekly AIEO prompt-snapshot cron. For each active prompt × active engine,
 * runs the engine's query() and writes one row to seo_prompt_snapshots.
 * Idempotent per UTC day; budget-guarded.
 *
...
import {
  ...
  loadBudgetState,
  recordSpend,
} from "@/lib/aieo/budget";
import {
  getEngineAdapter,
  listRegisteredEngineIds,
} from "@/lib/aieo/engines/registry";
import { type AieoSecretKey, getSecret } from "@/lib/aieo/secrets/store";
import { classifySentiment } from "@/lib/aieo/sentiment";
import { getSetting } from "@/lib/aieo/settings/store";
import { listTopCompetitorDomains } from "@/lib/dal/seo/competitors";
import { listEngines } from "@/lib/dal/seo/engines";
import {
  hasPromptSnapshotOnDate,
  insertPromptSnapshot,
} from "@/lib/dal/seo/prompt-snapshots";
import { getPromptById, listPrompts } from "@/lib/dal/seo/prompts";
