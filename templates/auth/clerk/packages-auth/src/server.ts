/**
 * @repo/auth/server - Clerk server implementation
 */
import "server-only";
...
// Helper functions for user migration
...
export async function checkUserExistsInClerk(
  ...
  try {
    ...
    console.error("[Clerk] Error checking user existence:", error);
    return false;
  }
}
...
export async function createUserInClerk(
  ...
  try {
    ...
    if (err?.errors?.[0]?.code === "form_identifier_exists") {
      console.log("[Clerk] User already exists in Clerk");
      return null;
    }
    console.error("[Clerk] Error creating user in Clerk:", error);
    throw error;
  }
}
  ...
  try {
    ...
    if (users.length === 0) {
      ...
      try {
        ...
        console.error("[Clerk] Error syncing user to DB:", error);
      }
      return null;
    }
...
export async function signInEmail(params: {
  ...
  try {
    ...
    // Check if user exists locally with credentials
    ...
    // Check if user exists in Clerk, migrate if not
...
export async function signUpEmail(_params: {
  email: string;
  password: string;
  name: string;
  ...
  return {
    error: {
      message:
        "Clerk sign-up must be handled client-side. Use signUp.email() from @repo/auth/client instead.",
