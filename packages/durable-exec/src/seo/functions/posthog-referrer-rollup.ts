/**
 * Daily PostHog AI-referrer rollup cron.
 * Cron: daily 03:30 UTC (`30 3 * * *`)
 */

import { upsertReferrerDay } from "@repo/database/dal/seo";
...
async function fetchPosthogReferrers(opts: {
  ...
  const results: Array<{ source: string; sessions: number; signups: number }> =
    [];
  ...
  for (const referrerHost of AI_REFERRER_HOSTS) {
    ...
    results.push({ source: referrerHost, sessions: Number(sessions), signups });
  }
...
export const seoPosthogReferrerRollupFunction = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    const rows = await step.run("fetch-posthog", () =>
      fetchPosthogReferrers({ apiKey, projectId: projectId, host, date }),
    );
