/**
 * @repo/auth/server - Unified dynamic server auth (convenience wrapper)
 * Import via: import { auth, baseServer } from "@repo/auth/server"
 *
  ...
  private async loadProvider(): Promise<AuthServerProvider> {
    ...
    this.loading = (async () => {
      ...
      switch (providerName) {
        ...
        case "clerk":
        case "clerk-dev": {
          const { ClerkServer } = await import("./providers/clerk/server");
