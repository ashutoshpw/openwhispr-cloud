/**
 * @repo/auth/client - Unified dynamic client auth (convenience wrapper)
 * Import via: import { signIn, signOut, useSession } from "@repo/auth/client"
 *
 * This module dynamically loads the configured provider based on NEXT_PUBLIC_AUTH_PROVIDER.
 * For tree-shaking, import directly from the provider module instead:
  ...
  useSession(): UseSessionResult {
    ...
    try {
      switch (providerName) {
        ...
        case "clerk":
        case "clerk-dev": {
          const { useSession } = require("./providers/clerk/client");
          return useSession();
        }
        default:
          throw new Error(`Unknown auth provider: ${providerName}`);
