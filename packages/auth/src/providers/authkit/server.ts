/**
 * @repo/auth/authkit/server - WorkOS AuthKit server implementation
 * Import via: import { AuthKitServer } from "@repo/auth/authkit/server"
 */
import "server-only";
...
import { mapAuthKitSession } from "../../utils";
import type {
  AuthServerProvider,
  UnifiedSession,
  SignInResult,
...
export class AuthKitServer implements AuthServerProvider {
  ...
  async getSession(headers: Headers): Promise<UnifiedSession | null> {
    try {
      ...
      console.error("[AuthKitServer] Error getting session:", error);
      return null;
    }
  }
