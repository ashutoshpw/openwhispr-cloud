/**
 * @repo/auth - Configuration helpers
 * Import via: import { getProviderName, getAuthConfig } from "@repo/auth/config"
 */
...
export function validateConfig(provider: AuthProvider): void {
  ...
  if (provider === "clerk" || provider === "clerk-dev") {
    if (!process.env.NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY) {
      throw new Error(
        "NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY environment variable is required for clerk provider",
      );
    }
    if (!process.env.CLERK_SECRET_KEY) {
      throw new Error(
        "CLERK_SECRET_KEY environment variable is required for clerk provider",
      );
    }
  }
}
